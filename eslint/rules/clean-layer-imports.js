...
function isImportAllowed(fromLayer, toLayer) {
    if (!fromLayer || !toLayer)
        return true; // unknown layers are ignored
    if (fromLayer === toLayer)
        return true; // Slef reference is OK
        ...
        return {
            ImportDeclaration(node) {
                ...
                if (importPath.startsWith(".")) {
                    ...
                    resolvedImportPath = path.resolve(projectRoot, importPath.replace(/^@\/?/, ""));
                }
                ...
                if (!isImportAllowed(fromLayer, toLayer, layerConfig)) {
                    context.report({
                        node,
                        message: `${"Import violation:"} ${chalk.green(fromLayer)} → ${chalk.red(toLayer)} : ${chalk.cyan('import "' + node.source.value + '"')}`,
                    });
                }
            },
        };
